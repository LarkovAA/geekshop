...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        action_2__condition = Q(order__update__gt=F('order__created') + action_1__time_delta) & Q(order__update__lte=F('order__created') + action_2__time_delta)
        ...
        for orderitem in test_orderss:
            t_list.add_row(
                [f'{orderitem.action_order} заказ №{orderitem.order.pk}'f'{orderitem.product.name:15}',
